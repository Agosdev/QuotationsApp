...
import { createQuoteAppData } from "../graphql/mutations";
export default function QuoteAppDataCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    setQueryName(initialValues.queryName);
    setQuotesGenerated(initialValues.quotesGenerated);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await API.graphql({
            query: createQuoteAppData.replaceAll("__typename", ""),
            variables: {
              input: {
                ...modelFields,
              },
      ...
      {...getOverrideProps(overrides, "QuoteAppDataCreateForm")}
      {...rest}
    >
      <TextField
        label="Query name"
